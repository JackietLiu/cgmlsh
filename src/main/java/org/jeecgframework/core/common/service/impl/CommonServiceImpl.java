...
public class CommonServiceImpl implements CommonService {
	...
	public void addcount(String funcid ) {
		String sqlcount="select id from t_s_function_clickcount where functionid='"+funcid+"' and userid='" + ResourceUtil.getSessionUser().getId()+"'";
		String theid=this.getSingleValue(sqlcount) ;
		 
		if(StringUtil3.isBlank(theid))
		{
			String sqlinsert="insert into t_s_function_clickcount(id,functionid,userid,clickcount,create_date) "
					+ " values('" +StringUtil3.getUUID()+"','" 
					+funcid +"','"
					+ResourceUtil.getSessionUser().getId() +"',1,now() )";
			this.updateBySqlString(sqlinsert);
		}
		else
		{
			String sqlupdate="update t_s_function_clickcount set  clickcount= clickcount +1 "
					+ " where id='" + theid +"'";
			this.updateBySqlString(sqlupdate);
		}
		//String sql="insert into t_s_function_clickcount(id," ;
		 
	}
	/**
	 * zczadd 
	 */
		public void getDataGridReturnBySql(String sql1,String sql2,final CriteriaQuery cq,
				final boolean isOffset) {
			commonDao.getDataGridReturnBySql(sql1,sql2,cq, isOffset);
		...
		{
			return this.commonDao.getNextValueByName(seq_name);
		}
		 /**
		  * zczadd 
		  */
		...
		public String getConfigByCode(String code) {
			return  commonDao.getConfigByCode(code);
		}
		/**
		 * zczadd
		 * @param sql
		...
		public List<TbConfigEntity> getConfigByGroupflag(String groupflag) {
			return  commonDao.getConfigByGroupflag(groupflag);
		}
		/**
		 * zczadd
		 * @param sql
		...
		public String getConfigById(String id) {
			return  commonDao.getConfigById(id);
		}
		
		public <T> T findObjForJdbcSingle(String sql , Class<T> clazz) {
			return commonDao.findObjForJdbcSingle(sql, clazz);
		}

...
public class WeixinLong2shortController extends BaseController {
	...
	public void exportXls(WeixinLong2shortEntity weixinLong2short,HttpServletRequest request,HttpServletResponse response
		...
		try {
			...
			//#########zczchanged 
			workbook = (HSSFWorkbook) ExcelExportUtil.exportExcel(new ExportParams("长连接转短链接列表", "导出人:"+ResourceUtil.getSessionUserName().getRealName(),
	...
	public AjaxJson importExcel(HttpServletRequest request, HttpServletResponse response) {
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			//params.setSecondTitleRows(1);zcznoted
			params.setHeadRows(1);
			params.setNeedSave(true);
			try {
				List<WeixinLong2shortEntity> listWeixinLong2shortEntitys = 
					ExcelImportUtil.importExcelBySax(file.getInputStream(),WeixinLong2shortEntity.class,params);

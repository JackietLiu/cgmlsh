...
<body>
						...
						<div class="panel-body">
							<form id="searchForm" onkeydown="if(event.keyCode==13){doSearch();return false;}">
											<div class="col-xs-12 col-sm-6 col-md-4">
												<label for="procname">名称：</label>
												<div class="input-group" style="width: 100%">
													<input type="text" class="form-control input-sm" id="procname" name="procname"/>
												</div>
											</div>
											<div class="col-xs-12 col-sm-6 col-md-4">
									    		<label for="isactive">在用：</label>
									    		<div class="input-group" style="width:100%">
										    		<select class="form-control input-sm" id="isactive" name="isactive"></select>
									    		</div>
								    		</div>
	...
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="add('录入','tShRuleInfoProcController.do?goAdd','tShRuleInfoProcList',600,400)" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>
			<button onclick="update('编辑','tShRuleInfoProcController.do?goUpdate','tShRuleInfoProcList',600,400)" id="btn_edit" class="btn btn-success btn-sm">
				<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑
			</button>
			<button onclick="deleteALLSelect('批量删除','tShRuleInfoProcController.do?doBatchDel','tShRuleInfoProcList',600,400)" id="btn_delete" class="btn btn-danger btn-sm">
				<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> 批量删除
			</button>
			<button onclick="detail('查看','tShRuleInfoProcController.do?goUpdate','tShRuleInfoProcList',600,400)" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-search" aria-hidden="true"></span> 查看
			</button>
			<button onclick="ImportXls('导入','','tShRuleInfoProcList',600,400)" id="btn_download" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-download" aria-hidden="true"></span> 导入
			</button>
			<button onclick="ExportXls('导出','','tShRuleInfoProcList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span> 导出
			</button>
			<button onclick="ExportXlsByT('模板下载','','tShRuleInfoProcList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			Promise.all(promiseArr).then(function(results) {
				loadSearchFormDicts($("#searchForm").find("select[name='isactive']"),"","isactive","select","在用");
			...
			$('#tShRuleInfoProcList').on(
					'check.bs.table uncheck.bs.table load-success.bs.table '
							+ 'check-all.bs.table uncheck-all.bs.table',
					function() {
						$('#btn_delete').prop('disabled',!$('#tShRuleInfoProcList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#tShRuleInfoProcList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#tShRuleInfoProcList').bootstrapTable({
									url : 'tShRuleInfoProcController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#tShRuleInfoProcList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#tShRuleInfoProcList').bootstrapTable('getOptions').pageNumber;
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var href = '';
													href += "<a href='javascript:void(0);'  class='ace_button'  onclick=delObj('tShRuleInfoProcController.do?doDel&id="
															+ row.id
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,procname,isactive,procmysql,memo,updateName,updateDate,createName,createDate,'
				};
		...
		function reloadTable() {
			$('#tShRuleInfoProcList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'tShRuleInfoProcController.do?upload',"tShRuleInfoProcList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("tShRuleInfoProcController.do?exportXls","tShRuleInfoProcList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("tShRuleInfoProcController.do?exportXlsByT","tShRuleInfoProcList");
		}
